...
```{r}
x_test_cout = subset(test_cout, select=-c(PolicyID, ClaimNb, ClaimAmount))
y_test_cout = test_cout$ClaimAmount

RMSE <- function(observed_data, predicted_data){
  return(sqrt(mean((observed_data - predicted_data)^2)))
}
y_gamma_cout = predict(glm_gamma_cout, newdata = x_test_cout, type = "response")
y_gaussien_cout = predict(glm_gaussien_cout, newdata = x_test_cout, type = "response")
y_bin_neg_cout = predict(glm_bin_neg_cout, newdata = x_test_cout, type = "response")

# Créer un data frame avec les noms de modèle et leur AIC associé
RMSE_data_cout <- data.frame(Modele = c("Modele Gaussien", "Modele Gamma", "Modele Binomial Negatif"), 
                          RMSE = c(RMSE(y_test_cout, y_gaussien_cout),
                                  RMSE(y_test_cout, y_gamma_cout),
                                  RMSE(y_test_cout, y_bin_neg_cout)))
...
Comme vu précédemment, nous allons retenir seulement la binomiale négative
...
```{r}
# Suppression des NA
base_data_lasso <- na.omit(base_data_onehot)
...
ltrain <- base_data_lasso[indexes, ]
ltest <- base_data_lasso[-indexes, ]
...
formula_model_freq <- ClaimNb ~ -1 + Exposure + CarAge + Gas + Density + DriverAge +
                      Power_1 + Power_2 + 
                      Brand_1 + Brand_2 +  
                      Region_1 + Region_2 + Region_3 

train_matrix_freq <- model.matrix(formula_model_freq, data = ltrain)
X_train_freq <- as.matrix(train_matrix_freq)
y_train_freq <- ltrain$ClaimNb

# Encodage des variables catégorielles en variables binaires pour l'échantillon de test
test_matrix_freq <- model.matrix(formula_model_freq, data = ltest)
X_test_freq <- as.matrix(test_matrix_freq)
y_test_freq <- ltest$ClaimNb
...
pred_gaussian_freq <- predict(lasso_gaussian_freq, s = "lambda.min", newx = X_test_freq, type = "response")
rmse_gaussian_freq <- RMSE(y_test_freq,pred_gaussian_freq)
...
# Prédictions et calcul du RMSE sur l'échantillon de test
pred_poisson_freq <- predict(lasso_poisson_freq, s = "lambda.min", newx = X_test_freq, type = "response")
rmse_poisson_freq <- RMSE(y_test_freq, pred_poisson_freq)
...
rmse_values_freq <- c(Gaussian = rmse_gaussian_freq, Poisson = rmse_poisson_freq)
...
```{r}
#Modéle et splitage des données. On ne se concentre que sur les valeurs positives
formula_model <- ClaimAmount ~ -1 + Exposure + CarAge + Gas + Density + DriverAge +
                      Power_1 + Power_2 + 
                      Brand_1 + Brand_2 +  
                      Region_1 + Region_2 + Region_3 

ltrain_cout <- ltrain[ltrain$ClaimAmount > 0, ]
ltest_cout <- ltest[ltest$ClaimAmount > 0, ]

# Encodage des variables catégorielles en variables binaires pour l'échantillon d'apprentissage
train_matrix_cout <- model.matrix(formula_model, data = ltrain_cout)
X_train_cout <- as.matrix(train_matrix_cout)
y_train_cout <- ltrain_cout$ClaimAmount

# Encodage des variables catégorielles en variables binaires pour l'échantillon de test
test_matrix_cout <- model.matrix(formula_model, data = ltest_cout)
X_test_cout <- as.matrix(test_matrix_cout)
y_test_cout <- ltest_cout$ClaimAmount
